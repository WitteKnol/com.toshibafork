        ...
        if (device) {
          switch (command) {
            ...
            case Constants.CMD_FCU_FROM_AC:
              // Confirmed via live AMQP capture: right after acknowledging
              // a locally issued command (e.g. off: 0x31), the AC/cloud can
              // send a second, contradicting push a second or two later
              // (e.g. onoff back to 0x30) with self-cleaning explicitly
              // false - not a self-cleaning report, just a stale/duplicate
              // status. Apply the same grace-period guard already used for
              // the HTTP status poll so this push can't undo the command we
              // just sent.
              if (device.recentlyCommandedLocally()) {
                this.Driver.homey.app.logStates(`Skipped (recently commanded): ${device.getData().DeviceUniqueID} ${device.getName()} ${command} ${payload.payload.data}`);
              } else {
