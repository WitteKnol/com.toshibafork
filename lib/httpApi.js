...
module.exports = class HttpApi extends SimpleClass {
  ...
  async getACStatus(deviceId) {
    ...
    const params = {
      consumerId: this.token_information.consumerId,
      ACId: deviceId,
    };
  ...
  async login(username, password, deviceId) {
   ...
   const data = await this.doPostRequest(url, json, false, { 'Device-ID': deviceId });
    ...
    this.homey.settings.set(
      Constants.SettingTokenInformation,
      this.token_information,
    );
  ...
  async getEnergyConsumption(deviceIDs, type, from, to, timezone) {
  ...
  setHeaderAndParams() {
    ...
    this.header = {
      Authorization: `Bearer ${this.token_information.access_token}`,
    };
  }
