...
function logError(device, error) {
  device.homey.app.logInformation(device.getName(),
    {
...
async function addCapabilityToDevice(device, type, name) {
  if (!device.hasCapability(name)) {
    await device.addCapability(name).catch(error => logError(device, error));
    // add listener
    device.registerCapabilityListener(name, async (value, opts) => {
      await device.setCapabilityValue(name, value).catch(error => logError(device, error));
      await device.updateStateAfterUpdateCapability(new Set([name])).catch(error => logError(device, error));
    });
  }
  if (type) {
    await device.setStoreValue(type, name).catch(error => logError(device, error));
  }
}
...
async function determineMeritA(device, bit0, bit2, bit5, type) {
  ...
  if (bit5) {
    ...
    await device.addCapability(Constants.CapabilityHas8C).catch(error => logError(device, error));
    await device.setCapabilityOptions(
    ...
    await device.addCapability(Constants.CapabilityHasNo8C).catch(error => logError(device, error));
  }
  await device.setStoreValue(Constants.StoredValuesMeritA, meritA).catch(error => logError(device, error));
}

async function determineMeritB(device, bit) {
  if (bit) {
    await addCapabilityToDevice(device, '', Constants.CapabilityTargetMeritB).catch(error => logError(device, error));
    meritB.push(Constants.MeritB_Fire_Place_1);
    meritB.push(Constants.MeritB_Fire_Place_2);
  }
  await device.setStoreValue(Constants.StoredValuesMeritB, meritB).catch(error => logError(device, error));
}
...
exports.setCapabilities = async device => {
  ...
  const meritFeatureBitString = parseInt(MeritFeature, 16)
    .toString(2)
    .padStart(16, '0');
  ...
  await determineACMode(device, meritBit6, meritBit7).catch(error => logError(device, error));
  await determineSwingMode(device, meritBit1, meritBit14, type).catch(error => logError(device, error));
  await determineAirPureIon(device, meritBit3).catch(error => logError(device, error));
  await determineMeritA(device, meritBit0, meritBit2, meritBit5, type).catch(error => logError(device, error));
  await determineMeritB(device, meritBit4).catch(error => logError(device, error));
  await determineEnergyConsumption(device, meritBit15).catch(error => logError(device, error));
};
