...
module.exports = class EnergyConsumption extends SimpleClass {
  ...
  async getNextEnergyConsumption(device, date) {
    ...
    if( newEnergy !== 0 ){
      // Neither awaited nor caught here previously - a storage error (e.g.
      // SQLITE_CORRUPT/SQLITE_IOERR on the Homey Pro's own disk) became an
      // unhandled promise rejection that crashed the whole app, since this
      // function has already returned by the time either promise settles.
      device.setStoreValue( Constants.StoredEnergyPreviousHourEnergy, newEnergy ).catch(error => logError(device, error));
      device.setStoreValue( Constants.StoredEnergyPreviousHour, hour ).catch(error => logError(device, error));
    }
